...
// RESUMEN CORREGIDO: Se extrae la primera posición [0] aquí mismo en el Backend
    ...
    db.query('SELECT * FROM periodos WHERE id = ?', [pId], (err, periodos) => {
        ...
        db.query('SELECT * FROM ingresos WHERE periodo_id = ? ORDER BY fecha DESC', [pId], (err, ingresos) => {
            db.query('SELECT * FROM egresos WHERE periodo_id = ? ORDER BY fecha DESC', [pId], (err, egresos) => {
                ...
                `, [pId, pId], (err, totales) => {
                    res.json({ 
                        periodo: periodos[0], // <-- LA CORRECCIÓN CLAVE: Pasamos el objeto plano sin corchetes
                        ingresos: ingresos, 
                        egresos: egresos, 
                        resumen: totales[0] || { total_ingresos: 0, total_egresos: 0 } // <-- También extraemos los totales
                    });
                });
            });
        });
...
app.post('/api/ingresos', (req, res) => {
    ...
    db.query('INSERT INTO ingresos (periodo_id, descripcion, monto, fecha) VALUES (?, ?, ?, ?)', 
...
app.post('/api/egresos', (req, res) => {
    const { periodo_id, descripcion, monto, fecha } = req.body;
    db.query('INSERT INTO egresos (periodo_id, descripcion, monto, fecha) VALUES (?, ?, ?, ?)', 
    [periodo_id, descripcion, monto, fecha], () => {
        res.json({ success: true });
    });
});
    ...
    db.query('SELECT * FROM periodos WHERE id = ?', [pId], (err, periodos) => {
        ...
        db.query('SELECT * FROM ingresos WHERE periodo_id = ?', [pId], (err, ing) => {
            db.query('SELECT * FROM egresos WHERE periodo_id = ?', [pId], (err, egr) => {
                ...
                ing.forEach(i => {
                    const f = i.fecha.toISOString ? i.fecha.toISOString().split('T')[0] : i.fecha;
                    doc.fillColor('#333').text(`${f} - ${i.descripcion}: +$${parseFloat(i.monto).toFixed(2)}`);
                });
                
                doc.moveDown(1).fillColor('#c0392b').text('--- DETALLE DE EGRESOS ---');
                egr.forEach(e => {
                    const f = e.fecha.toISOString ? e.fecha.toISOString().split('T')[0] : e.fecha;
                    doc.fillColor('#333').text(`${f} - ${e.descripcion}: -$${parseFloat(e.monto).toFixed(2)}`);
                });
